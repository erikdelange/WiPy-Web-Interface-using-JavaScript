...
def query(request):
    ...
    :param bytes request: the complete HTTP request line.
...
def request(line):
    ...
    :raises InvalidRequest: if line does not contain 3 components separated by
                            spaces, no other checks done here
    """
    d = {key: value for key, value in zip(["method", "url", "version"], line.decode("utf-8").split())}

    if len(d) != 3:
        raise InvalidRequest(line)
